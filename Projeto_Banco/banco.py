...
def efetuar_deposito() -> None:
    if len(contas) > 0:
        ...
        if conta:
            valor: float = float(input('Informe o valor do deposito: R$ '))
...
def efetuar_transferencia() -> None:
    if len(contas) > 0:
        numero_origem: int = int(input('Informe o número de sua conta: '))
        ...
        if conta_origem:
            ...
            if conta_destino:
                ...
                print(f'A conta destino com número de {numero_destino} não foi encontrada.')
        else:
